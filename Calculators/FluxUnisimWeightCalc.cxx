...
namespace evwgh {
  ...
  std::vector<std::vector<double> > FluxUnisimWeightCalc::GetWeight(bsim::Dk2Nu & e)
  {
    ...
    for(unsigned int inu = 0; inu < 1; inu++){
     ...
     // Discover the neutrino parent type
      ...
      if (      e.decay.ptype==13  || e.decay.ptype==-13  ) ptype = 0;
      else if ( e.decay.ptype==211 || e.decay.ptype==-211 ) ptype = 1;
      else if ( e.decay.ptype==130                               ) ptype = 2;
      else if ( e.decay.ptype==321 || e.decay.ptype==-321 ) ptype = 3;                                    
      else {
	throw cet::exception(__FUNCTION__) << GetName()<<"::Unknown ptype "<<e.decay.ptype<< std::endl;
      ...
      if (      e.decay.ntype== 12  ) ntype=0;
      else if ( e.decay.ntype==-12  ) ntype=1;
      else if ( e.decay.ntype== 14  ) ntype=2;
      else if ( e.decay.ntype==-14  ) ntype=3;
      else {
	throw cet::exception(__FUNCTION__) << GetName()<<"::Unknown ntype "<<e.decay.ntype<< std::endl;
      }
  ...
  std::vector< double > FluxUnisimWeightCalc::MiniBooNERandomNumbers(std::string pdg){
    ...
    if(pdg.compare("horncurrent") == 0){	 
      ...
      throw cet::exception(__FUNCTION__) << GetName()
					 << "Beam Unisim MiniBooNE random numbers are not configured for " << pdg<<std::endl;	  	
    }
